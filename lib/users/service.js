...
const createUser = Promise.coroutine(function* (attributes) {
  ...
  return Bookshelf.transaction(Promise.coroutine(function* (t) {
    ...
    if (role) {
      yield user.related("roles").create({ name: role }, options);
    }
...
const updateUser = (userId, attributes) =>
  User.where({ id: userId }).save(attributes, { patch: true });

const getUsers = (user, { roles, enrolled, pageSize, page, sort }) =>
  User.query((qb) => {
    if (roles) {
...
const getUser = (user, id) =>
  User.forge({ id }).fetch({ withRelated: ["roles"] });
